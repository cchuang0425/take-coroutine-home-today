...
各位想像一下，一步做完，再下一步，這樣的世界多麼單純美好。我們可以看到上述問題寫成程式大概是這樣
...
我們把每一個動作都當作一個 event，並且用 command pattern 進行包裝，於是在原先簡單的程式裡，我們變成需要使用額外的暫存空間保存程式執行的過成，每個 command 與原始的 n 的對應關係，還有需要一條等待事情完成的 thread。

(程式脈絡已經與先前完全不同！)

從這裡我們可以看到，整個程式的脈絡已經完全不同。

然而，這算是其次，更重要的問題是，

(困難的，不是把它寫出來，而是寫完後要如何維護與擴充！)

當邏輯與 command 間的關係散落在各處時，後續擴充與維護會變得非常複雜。雖然，如果一切重頭來，一開始就知道會有非同步架構的需要的話，我們或許可以使用合適的抽象概念來解決它，例如引入 Akka，但是我相信來到這裡的各位，其實大家可能所處的環境跟我們類似，也就是原本的邏輯是同步式的，或你原本想用的機制就是同步式的。然而碰到了這種非同步的環境，有的人起步還早，可以立馬煞車轉彎，有的人可能希望重複利用過去寫出來的程式，不希望有太大的修改，這也是我們今天的主題，希望 Quasar 能幫到有這方面需要的人。
...
沒錯，當我們拋出一個 checked exception 時，它會中斷執行，但中斷不代表執行的 context 就不見了，它會被保存起來。Java 沒有辦法使用 coroutine 的點就在於，這個中斷時的 context 並不是被某個框架或工具給保留了，而是被 JVM 給拿去管了。於是 Quasar 的機制就在於，它能夠把這個中斷當下的 context 保存在它自己的資料結構裡頭，然後把控制權交給開發者，你就能決定什麼時候要回到這段程式裡繼續執行了。
...
首先，我們先設定建構工具，Quasar 對 Gradle 支援較好，而剛好我們團隊用的也是 Gradle，所以我們今天先以 Gradle 來說明，Quasar 官網也有 Maven 的設定說明，使用 Maven 的人可以參考 Quasar 的文件。
...
這裡會這樣寫，是不合格的，首先，Channel 像 IO Stream 一樣，都有 auto closable 的實作，所以可以用 try 的語法來包裝，其次，其實我們只是為了呼應前面說到非同步環境下的同步程式，所以用這種寫法，其實你自己在寫時，這樣會更好
...
到此，大家已經知道整個概念是如何實作的了，我們來執行看看結果如何
