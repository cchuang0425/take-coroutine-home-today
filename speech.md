...
各位今年聽了 Kotlin 的 coroutine 場次了嗎？Kotlin 語言去年加入了 coroutine 這個另人激賞的機制，我想很多人都想用它來解決開發 Android App 或是 Server backend 的並行問題。
...
我們今天的脈絡，會從我們團隊遇到的問題出發，我知道這與各位實際遇到的問題一定不會完全一樣，但是我們把這解法放在某個特定處境中，會讓各位更深刻的理解這個解法與它背後的思考。

提出了問題後，我們會歸納出在這問題中所反映的需求，以及從需求看見解決的機制，接下來我們會看到這機制如何實踐，以及驗證其執行結果。

我們這個場次會看一些程式碼，而程式碼在最後一頁投影片中會告訴各位 GitHub 檔案庫的連結。

最後我們會深入討論 Quasar 這個框架的實務使用想法與延伸應用。

## 問題 → 需求

我們通常都是從問題裡產生需求的，我們舉個數學計算的例子，來對應到實際工作上更複雜的情況。
...
如果各位有用過電鍋會知道，東西放了之後，就不管它，然後等到它跳起來就好。可是原本的同步執行程式怎麼轉換到這種場景呢？
...
我很想帶大家看程式碼，真的，但我現在不打算帶大家看程式碼，建議各位可以參考範例程式的內容，我們回到先前那段程式來說明，簡而言之，我會把每一個動作都當作一個 event，並且用 command pattern 進行包裝，因此，首先我們需要使用額外的暫存空間保存 command 的 reference、每個 command 與原始的 n 的對應關係，再者，我們需要在每個 command 裡指定下一步驟是什麼，跑完每個 command 時，需要組合這些經過的對應，而最後跑完時，還有需要一條等待事情完成的 thread。
